...
import { format, parseISO, eachDayOfInterval, startOfWeek, endOfWeek, isSameDay, subWeeks, addWeeks, startOfMonth, endOfMonth, subMonths, addMonths, isSameMonth } from 'date-fns';
...
const CalendarView: React.FC<CalendarViewProps> = ({ tasks, assignees = {}, onTaskClick }) => {
  ...
  return (
    <div className={styles['calendar-view__calendar']}>
      ...
      <div className={styles['calendar-view__viewport']}>
                  ...
                  <div key={day.toISOString()} className={styles['calendar-view__day-column']}>
                    {tasks.length === 0 ? (
                      ...
                      tasks.map((task) => (
                        <div
                          key={task.id}
                          className={`${styles['calendar-view__task']} ${task.status === 'done' ? styles['calendar-view__task--done'] : ''} ${task.status !== 'done' && new Date(task.due_date).getTime() < Date.now() ? styles['calendar-view__task--overdue'] : ''}`}
                        ...
                        >
                          ...
                          <div className={styles['calendar-view__task-meta']}>
                            <span className={styles['calendar-view__task-date']}>
                              {task.status === 'done'
                                ? 'Выполнена'
